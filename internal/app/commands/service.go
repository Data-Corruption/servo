		...
		Commands: []*cli.Command{
			{
				...
				Flags: []cli.Flag{
					...
					&cli.IntFlag{
						Name:  "port",
						Usage: "set dashboard HTTPS port",
					},
				},
				Action: func(ctx context.Context, cmd *cli.Command) error {
					updated := false
					...
					if _, err := config.Update(a.DB, func(cfg *types.Configuration) error {
						...
						if cmd.IsSet("port") {
							cfg.UIBind = fmt.Sprintf(":%d", cmd.Int("port"))
							updated = true
						}
						return nil
					}); err != nil {
			...
			},
			{
				...
				Action: func(ctx context.Context, cmd *cli.Command) error {
					...
					// optional port override (maps onto the HTTPS bind)
					if port := cmd.Int("port"); port != 0 {
						cfg.UIBind = fmt.Sprintf(":%d", port)
					}

					// build the chi handler once and share it across both listeners
					mux := router.New(a)
					...
					if a.ProxyServer != nil {
						go func() {
							if err := a.ProxyServer.Listen(); err != nil {
								a.Log.Errorf("proxy listener stopped with error: %v", err)
							}
						}()
						a.AddCleanup(func() error {
							return a.ProxyServer.Shutdown()
						})
					}
